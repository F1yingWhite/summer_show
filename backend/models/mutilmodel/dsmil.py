...
def init_model(
    ...
    i_classifier = FCLayer(in_size=feat_size, out_size=num_class).to(device)
    b_classifier = BClassifier(
        input_size=feat_size,
        output_class=num_class,
    ).to(device)
    milnet = MILNet(i_classifier, b_classifier).to(device)
    milnet.apply(lambda m: apply_sparse_init(m))
    # BinaryCrossEntropyLoss,主要用于二分类任务中
    ...
    # 余弦退火调度器,设置最大的epoch为50,到时候的最小学习率为0.000005
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, max_epoch, min_lr)
    return milnet, criterion, optimizer, scheduler


if __name__ == "__main__":
    # 初始化模型
    milnet, criterion, optimizr, scheduler = init_model()
    summary(milnet, input_size=(512, 4096))
