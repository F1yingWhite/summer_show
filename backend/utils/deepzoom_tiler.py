...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, mag_levels, base_mag, objective, format, tile_size, overlap, limit_bounds, quality, workers, threshold):
...
def patch_extraction(slide_path, overlap=0, format='jpeg', slide_format='svs', workers=40, quality=70, tile_size=224, base_mag=20, magnifications=(0, 2), objective=20, background_t=15):
    Image.MAX_IMAGE_PIXELS = None
    levels = tuple(sorted(magnifications))
    assert len(levels) <= 2, 'Only 1 or 2 magnifications are supported!'

    path_base = os.path.dirname(slide_path)
    if len(levels) == 2:
        out_base = os.path.join(path_base, 'pyramid')
    else:
        out_base = os.path.join(path_base, 'single')

    print('Process slide: {}'.format(slide_path))
    DeepZoomStaticTiler(slide_path, 'WSI_temp', levels, base_mag, objective, format, tile_size, overlap, True, quality, workers, background_t).run()
    nested_patches(slide_path, out_base, levels, ext=format)
    shutil.rmtree('WSI_temp_files')

    print('Patch extraction done for the slide: {}'.format(slide_path))
