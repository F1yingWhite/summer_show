...
export function Multimodal() {
  ...
  const handlePrediction = () => {
    ...
    axios.post('http://localhost:9002/api/multimodal/prediction')
      .then(res => {
        loadingMessage();
  ...
  const uploadProps = {
    name: 'file',
    action: 'http://localhost:9002/api/wsi/upload',
    headers: {
    ...
    customRequest: async ({ file, onSuccess, onError, onProgress }) => {
      ...
      try {
        await axios.post('http://localhost:9002/api/wsi/upload', formData, {
